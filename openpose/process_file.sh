...
# Define paths
HOST_VIDEO_PATH="/home/tim/Work/nexa/nexa-audio-video-pipelines/data/test/openpose_example/video.avi"  # Full path to the input video on your host
...
CONTAINER_ID=$(docker run -d --gpus all -v $(dirname "$HOST_VIDEO_PATH"):/input -v $HOST_OUTPUT_DIR:/output $CONTAINER_NAME /bin/bash -c "sleep infinity")
...
docker exec $CONTAINER_ID /bin/bash -c "cd /opt/openpose && ./build/examples/openpose/openpose.bin --video /input/'$(basename "$HOST_VIDEO_PATH")' --display $DISPLAY --render_pose $RENDER_POSE --net_resolution $NET_RESOLUTION --write_json /output"


# Try changing ./build/examples/openpose/openpose.bin to ./build/src/openpose/libopenpose.so.1.7.0
# How can I run it not in demo mode?
